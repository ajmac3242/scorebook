...
describe("Momentum Alert Logic", () => {
    describe("Opponent Run Detection", () => {
        it("detects an 8-0 run", () => {
            const stats = [
                { type: ACTION_TYPES.MAKE, points: 3, playerId: SPECIAL_PLAYER_IDS.OPPONENT },
                { type: ACTION_TYPES.MAKE, points: 2, playerId: SPECIAL_PLAYER_IDS.OPPONENT },
                { type: ACTION_TYPES.MAKE, points: 3, playerId: SPECIAL_PLAYER_IDS.OPPONENT },
            ];
        ...
        it("ignores deleted events", () => {
             const stats = [
                { type: ACTION_TYPES.MAKE, points: 3, playerId: SPECIAL_PLAYER_IDS.OPPONENT },
                { type: ACTION_TYPES.MAKE, points: 2, playerId: SPECIAL_PLAYER_IDS.OPPONENT },
                { type: ACTION_TYPES.MAKE, points: 3, playerId: SPECIAL_PLAYER_IDS.OPPONENT, deletedAt: 'today' },
            ];
    ...
    describe("Scoring Drought Detection", () => {
        ...
        it("detects drought within the same period", () => {
            const stats = [
                { type: ACTION_TYPES.MAKE, points: 2, playerId: "p1", period: 1, clockTime: 500 }
            ];
            // 500 - 300 = 200s (3m 20s)
        ...
        it("detects drought across periods", () => {
            const stats = [
                { type: ACTION_TYPES.MAKE, points: 2, playerId: "p1", period: 1, clockTime: 100 } // 1:40 left in P1
            ];
