  ...
  async pushUpdates() {
      ...
      try {
          ...
          for (const s of seasons) {
              const res = await fetch("/seasons", { method: "POST", headers, body: JSON.stringify(s) });
          ...
          for (const t of teams) {
              const res = await fetch("/teams", { method: "POST", headers, body: JSON.stringify(t) });
          ...
          for (const p of players) {
              const res = await fetch("/players", { method: "POST", headers, body: JSON.stringify(p) });
          ...
          for (const item of tp) {
              const res = await fetch(`/teams/${item.teamId}/players`, { method: "POST", headers, body: JSON.stringify(item) });
          ...
          for (const g of games) {
              const res = await fetch("/games", { method: "POST", headers, body: JSON.stringify(g) });
              if (res.ok) {
          ...
          for (const st of stats) {
              const res = await fetch(`/games/${st.gameId}/stats`, { method: "POST", headers, body: JSON.stringify(st) });
