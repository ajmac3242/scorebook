...
describe("mathUtils", () => {
  ...
  describe("formatTimestampToTime", () => {
    ...
    it("handles unexpected strings by slicing blindly (per implementation)", () => {
  ...
  describe("calculateElapsedMinutes", () => {
    it.each([
      ...
      [1, 0, "HALVES", 20],
      [2, 1200, "HALVES", 20],
      [3, 0, "HALVES", 60], // OT1 start/end (depends on how caller uses it)
      [3, 300, "QUARTERS", 25], // Middle of P3
    ])(
    ...
    it("handles default period type", () => {
      expect(calculateElapsedMinutes(1, 0)).toBe(10);
    });
  });
