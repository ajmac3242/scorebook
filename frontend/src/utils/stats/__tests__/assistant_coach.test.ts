...
import { calculateDefensiveIntegrity, calculateSituationalStats } from '../analytics';
...
describe('Assistant Coach Analytics', () => {
  describe('calculateDefensiveIntegrity', () => {
    it('aggregates points allowed by breakdown reason', () => {
      const stats: any[] = [
        { playerId: 'OPPONENT:1', type: ACTION_TYPES.MAKE, points: 2, breakdownReason: 'Missed Rotation' },
        { playerId: 'OPPONENT:2', type: ACTION_TYPES.MAKE, points: 3, breakdownReason: 'Missed Rotation' },
        { playerId: 'OPPONENT:1', type: ACTION_TYPES.MAKE, points: 2, breakdownReason: 'Transition Leak' },
  ...
  describe('calculateSituationalStats', () => {
    it('aggregates PPP and eFG% by situation for our team only', () => {
      const stats: any[] = [
        { playerId: 'PLAYER:1', type: ACTION_TYPES.MAKE, points: 2, situation: 'ATO' },
        { playerId: 'PLAYER:1', type: ACTION_TYPES.MISS, points: 0, situation: 'ATO' },
        { playerId: 'OPPONENT:1', type: ACTION_TYPES.MAKE, points: 2, situation: 'ATO' }, // Should be ignored (opponent)
      ];
