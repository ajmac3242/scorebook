...
describe('AppDatabase schema', () => {
  ...
  it('cascades correctly when querying stats by gameId', async () => {
    await db.stats.bulkAdd([
      { id: 's1', gameId: 'g1', playerId: 'p1', type: 'FIELD_GOAL', period: 1, timestamp: new Date().toISOString() },
      { id: 's2', gameId: 'g1', playerId: 'p2', type: 'ASSIST', period: 1, timestamp: new Date().toISOString() },
      { id: 's3', gameId: 'g2', playerId: 'p1', type: 'REBOUND', period: 1, timestamp: new Date().toISOString() },
    ] as any);
  ...
  it('migrates data correctly from version 25 to current version', async () => {
    ...
    await v25db.open();
    await v25db.table('opponents').add({ id: 'o1', name: 'Legacy Opponent', roster: [] });
    await v25db.close();
    ...
    const archivedOpponents = await currentDb.opponents.where('isArchived').equals(1).toArray();
