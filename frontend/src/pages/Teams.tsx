...
import { db, type Team, type TeamPlayer, type Season, type Player } from '../db';
...
const Teams: React.FC = () => {
  ...
  const handleAddPlayerToTeam = async (playerId: string) => {
    ...
    const exists = teamPlayers.find(tp => tp.teamId === activeTeamId && tp.playerId === playerId);
    if (exists) return;
  ...
  const getPlayersForTeam = (teamId: string) => {
    const playerIds = teamPlayers.filter(tp => tp.teamId === teamId.toString()).map(tp => tp.playerId);
    ...
    <Box>
      ...
      {selectedSeasonId && (
        <Box>
          ...
          <Stack spacing={2}>
            {teams.length === 0 && <Typography>No teams in this season.</Typography>}
            {teams.map((team) => (
              <Paper key={team.id} className="moleskine-card">
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                  {getPlayersForTeam(team.id?.toString() || '').map(p => (
                    <Chip key={p.id} label={`${p.name} (#${p.defaultNumber})`} variant="outlined" />
                  ))}
                  {getPlayersForTeam(team.id?.toString() || '').length === 0 && (
      ...
      <Dialog open={openRosterDialog} onClose={() => setOpenRosterDialog(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Typography variant="body2" gutterBottom>Select players from the master list to add to this team.</Typography>
          <List>
            {players.map((player) => {
              const isInTeam = teamPlayers.some(tp => tp.teamId === activeTeamId && tp.playerId === player.id?.toString());
              return (
                <ListItem
                  key={player.id}
                  secondaryAction={
                ...
                >
                  <ListItemText primary={player.name} secondary={`#${player.defaultNumber}`} />
                </ListItem>
              );
            })}
          </List>
