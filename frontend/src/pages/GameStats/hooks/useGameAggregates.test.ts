...
describe("useGameAggregates", () => {
  ...
  it("calculates basic aggregates correctly", async () => {
    ...
    const p1Stats = result.current.aggregatedStats.find(
      (s) => String(s.id) === "p1",
    );
  ...
  it("calculates opponent play type efficiency", async () => {
    ...
    expect(result.current.opponentPlayTypeEfficiency).toHaveLength(1);
    expect(result.current.opponentPlayTypeEfficiency[0].type).toBe("ISO");
    expect(result.current.opponentPlayTypeEfficiency[0].points).toBe(6);
  });
  ...
  it("calculates play and shot clock efficiency", async () => {
    ...
    expect(
      result.current.shotClockEfficiency.find((s) => s.phase === "EARLY")
        ?.makes,
    ).toBe(1);
    expect(
      result.current.processEfficiency.find((s) => s.quality === "OPEN")?.makes,
    ).toBe(1);
  });
});
