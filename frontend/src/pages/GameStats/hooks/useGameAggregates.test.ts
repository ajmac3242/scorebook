...
describe("useGameAggregates", () => {
  ...
  it("calculates opponent play type efficiency with FTs and misses", () => {
    const oppStats = [
      { id: "o1", playerId: "OPPONENT", type: ACTION_TYPES.MAKE, points: 2, opponentPlayType: "PnR", timestamp: "1" },
      { id: "o2", playerId: "OPPONENT", type: ACTION_TYPES.MISS, points: 2, opponentPlayType: "PnR", timestamp: "2" },
      { id: "o3", playerId: "OPPONENT", type: ACTION_TYPES.MAKE, points: 1, opponentPlayType: "PnR", timestamp: "3" },
      { id: "o4", playerId: "OPPONENT", type: ACTION_TYPES.TURNOVER, opponentPlayType: "PnR", timestamp: "4" },
      { id: "o5", playerId: "OPPONENT", type: ACTION_TYPES.MAKE, points: 3, opponentPlayType: "PnR", timestamp: "5" },
    ];
    const { result } = renderHook(() => useGameAggregates({
    ...
    const pnr = result.current.opponentPlayTypeEfficiency.find(e => e.type === "PnR");
  ...
  it("triggers practice focus area alerts", () => {
    const badStats = [
      { id: "b1", playerId: "p1", type: ACTION_TYPES.MISS, points: 1, timestamp: "1" }, // 0% FT
      { id: "b2", playerId: "p1", type: ACTION_TYPES.TURNOVER, timestamp: "2" }, // High turnover
      { id: "b3", playerId: "OPPONENT", type: ACTION_TYPES.DEF_REBOUND, timestamp: "3" }, // Bad Oreb
    ];
    const { result } = renderHook(() => useGameAggregates({
  ...
  it("applies various quality and play filters in derived stats", () => {
    const filteredStats = [
      { id: "f1", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, shotQuality: "High", playName: "Hammer", timestamp: "1" },
      { id: "f2", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, shotQuality: "Low", playName: "Hammer", timestamp: "2" },
    ];
    const { result } = renderHook(() => useGameAggregates({
      rawData: { ...mockRawData, allStats: filteredStats },
      filters: { ...mockFilters, selectedQuality: "High", selectedPlay: "Hammer" }
    }));
  ...
  it("calculates shot clock efficiency", () => {
    const shotClockStats = [
      { id: "sc1", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, shotClockPhase: "EARLY", timestamp: "1" },
      { id: "sc2", playerId: "p1", type: ACTION_TYPES.MISS, points: 2, shotClockPhase: "EARLY", timestamp: "2" },
      { id: "sc3", playerId: "p1", type: ACTION_TYPES.MAKE, points: 3, shotClockPhase: "LATE", timestamp: "3" },
      { id: "sc4", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, timestamp: "4" }, // No phase
    ];
    const { result } = renderHook(() => useGameAggregates({
    ...
    const early = result.current.shotClockEfficiency.find(e => e.phase === "EARLY");
    expect(early?.attempts).toBe(2);
    expect(early?.makes).toBe(1);
    expect(early?.points).toBe(2);

    const late = result.current.shotClockEfficiency.find(e => e.phase === "LATE");
  ...
  it("calculates process efficiency (shot quality)", () => {
    const qualityStats = [
      { id: "q1", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, shotQuality: SHOT_QUALITY.OPEN, timestamp: "1" },
      { id: "q2", playerId: "p1", type: ACTION_TYPES.MISS, points: 2, shotQuality: SHOT_QUALITY.OPEN, timestamp: "2" },
      { id: "q3", playerId: "p1", type: ACTION_TYPES.MAKE, points: 3, shotQuality: SHOT_QUALITY.CONTESTED, timestamp: "3" },
    ];
    const { result } = renderHook(() => useGameAggregates({
    ...
    const open = result.current.processEfficiency.find(e => e.quality === SHOT_QUALITY.OPEN);
  ...
  it("calculates team data with various event types", () => {
    const teamStats = [
        { id: "t1", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, timestamp: "1" },
        { id: "t2", playerId: "p1", type: ACTION_TYPES.MISS, points: 1, timestamp: "2" }, // FTA
        { id: "t3", playerId: "p1", type: ACTION_TYPES.TURNOVER, timestamp: "3" },
        { id: "t4", playerId: "p1", type: ACTION_TYPES.OFF_REBOUND, timestamp: "4" },
    ];
    const { result } = renderHook(() => useGameAggregates({
  ...
  it("handles sorting in aggregated stats", () => {
    const sortStats = [
        { id: "p1", playerId: "p1", type: ACTION_TYPES.MAKE, points: 2, timestamp: "1" },
        { id: "p2", playerId: "p2", type: ACTION_TYPES.MAKE, points: 3, timestamp: "2" },
    ];
    const { result } = renderHook(() => useGameAggregates({
        rawData: { ...mockRawData, allStats: sortStats, players: [{ id: "p1", name: "P1" }, { id: "p2", name: "P2" }], teamPlayers: [{ playerId: "p1" }, { playerId: "p2" }] },
        filters: { ...mockFilters, sortConfig: { key: "points", direction: "asc" } }
    }));
