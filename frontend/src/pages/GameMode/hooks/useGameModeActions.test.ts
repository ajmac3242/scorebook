...
describe("useGameModeActions", () => {
  ...
  it("handles handleUndo", async () => {
    const stat = { id: "s1", gameId: "g1", type: ACTION_TYPES.MAKE, synced: 1 };
  ...
  it("handles handleEndGame", async () => {
    await mockDb.games.add({ id: "g1", teamId: "t1", synced: 1 });
    const { result } = renderHook(() => useGameModeActions(defaultParams));
  ...
  it("handles handleDeleteStat", async () => {
    await mockDb.stats.add({ id: "s1", gameId: "g1", type: ACTION_TYPES.MAKE, synced: 1 });
    const params = {
      ...defaultParams,
      statToDelete: "s1"
    };
  ...
  it("handles handleFlipPossessionArrow", async () => {
    await mockDb.games.add({ id: "g1", teamId: "t1", possessionArrow: "OUR_TEAM", synced: 1 });
    const params = {
      ...defaultParams,
