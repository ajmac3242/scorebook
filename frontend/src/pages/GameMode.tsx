    ...
    <Box sx={{ pb: 4 }}>
        ...
        <Grid item xs={12} md={4}>
          <Stack spacing={3}>
            ...
            <Paper className="moleskine-card">
              <Typography variant="subtitle2" gutterBottom sx={{ fontWeight: 600 }}>Active Lineup</Typography>
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1 }}>
                {players.map(p => (
                  <Button
                    key={p.id}
                    variant={selectedPlayerId === p.id?.toString() ? "contained" : "outlined"}
                    onClick={() => setSelectedPlayerId(p.id?.toString() || null)}
                    sx={{
                      justifyContent: 'flex-start',
                      px: 1,
                      py: 1,
                      borderColor: '#D1D1D1',
                      backgroundColor: selectedPlayerId === p.id?.toString() ? '#2D2D2D' : 'transparent',
                      color: selectedPlayerId === p.id?.toString() ? '#FFFDF5' : '#2D2D2D',
                    }}
                  >
                    <Avatar sx={{ width: 24, height: 24, fontSize: '0.75rem', mr: 1, bgcolor: selectedPlayerId === p.id?.toString() ? '#FFFDF5' : '#2D2D2D', color: selectedPlayerId === p.id?.toString() ? '#2D2D2D' : '#FFFDF5' }}>
                      {p.defaultNumber}
                    </Avatar>
      ...
      >
        ...
        <DialogContent>
          {!selectedPlayerId && (
             <Box sx={{ mb: 3 }}>
                <Typography variant="caption" gutterBottom sx={{ display: 'block', mb: 1 }}>Select Player First</Typography>
          ...
          <Typography variant="caption" gutterBottom sx={{ display: 'block', mb: 1 }}>What happened?</Typography>
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1 }}>
            <QuickAction type="MAKE" label="Make" icon={Check} />
            <QuickAction type="MISS" label="Miss" icon={Close} />
            <QuickAction type="REBOUND" label="Rebound" icon={SportsBasketball} />
              ...
              <Stack direction="row" spacing={1}>
                {[1, 2, 3].map(pts => (
                  <Button
                    ...
                    sx={{
                      borderColor: '#D1D1D1',
                      backgroundColor: points === pts ? '#2D2D2D' : 'transparent',
