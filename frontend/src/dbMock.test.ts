...
describe("dbMock", () => {
  it("throws an error when an unregistered table name is used", () => {
    expect(() => createTable("nonExistentTable")).toThrow("no config registered");
  });
  ...
  it("supports compound where queries", async () => {
    const table = createTable<{ id: string; gameId: string; type: string }>("stats");
    await table.add({ id: "s1", gameId: "g1", type: "FIELD_GOAL" });
    await table.add({ id: "s2", gameId: "g1", type: "ASSIST" });
    const results = await table.where("[gameId+type]").equals(["g1", "FIELD_GOAL"]).toArray();
