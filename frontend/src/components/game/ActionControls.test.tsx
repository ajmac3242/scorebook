...
describe("ActionControls", () => {
  ...
  it("calls callbacks when buttons are clicked", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /change possession to our team/i }),
    );
    expect(defaultProps.onTogglePossession).toHaveBeenCalled();

    await user.click(
      screen.getByRole("button", { name: /end and save game/i }),
    );
    expect(defaultProps.onEndGame).toHaveBeenCalled();

    await user.click(
      screen.getByRole("button", { name: /flip possession arrow/i }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: /record opponent turnover/i }),
    );
    expect(defaultProps.onOpponentTurnover).toHaveBeenCalled();

    await user.click(
      screen.getByRole("button", { name: /manage lineup substitutions/i }),
    );
    expect(defaultProps.onQuickSub).toHaveBeenCalled();

    await user.click(
      screen.getByRole("button", { name: /audit substitutions history/i }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: /record free throws/i }),
    );
  ...
  it("disables most buttons when isReadOnly is true", () => {
    ...
    expect(
      screen.getByRole("button", { name: /log team timeout/i }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: /record free throws/i }),
    ).toBeDisabled();
    ...
    expect(
      screen.queryByRole("button", { name: /end and save game/i }),
    ).not.toBeInTheDocument();
  });

  it("shows correct possession label based on state", () => {
    const { rerender } = render(
      <ActionControls {...defaultProps} possessionState="OUR_TEAM" />,
    );
    expect(
      screen.getByRole("button", { name: /change possession to opponent/i }),
    ).toBeInTheDocument();
