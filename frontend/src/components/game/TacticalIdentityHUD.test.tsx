...
describe("TacticalIdentityHUD", () => {
  const mockKpis: IdentityKPI[] = [
    { name: "stop_pct", value: 50.5, target: 60, label: "Stop %", isPercentage: true },
    { name: "efg", value: "45.0", target: 50, label: "eFG%", isPercentage: true },
  ...
  it("shows CheckCircle when target is met", () => {
    ...
    const paintTouchesBox = boxes.find(b => b.getAttribute("aria-label")?.includes("Paint Touches"));
    expect(paintTouchesBox?.querySelector('svg[data-testid="CheckCircleIcon"]')).toBeInTheDocument();

    const stopPctBox = boxes.find(b => b.getAttribute("aria-label")?.includes("Stop %"));
    expect(stopPctBox?.querySelector('svg[data-testid="CheckCircleIcon"]')).not.toBeInTheDocument();

    const tovBox = boxes.find(b => b.getAttribute("aria-label")?.includes("TOV"));
    expect(tovBox?.querySelector('svg[data-testid="CheckCircleIcon"]')).toBeInTheDocument();
  });

  it("handles empty or unknown KPI descriptions", () => {
    const unknownKpi: IdentityKPI[] = [{ name: "unknown", value: 1, target: 2, label: "Unknown" }];
