...
const DRILL_LIBRARY: Record<string, { label: string; drills: string[] }> = {
  TO: {
    label: "Ball Security",
    drills: [
      "Gauntlet Passing Drill",
      "2-on-3 Pressure Break",
      "Baseline Trap Escape",
    ],
  },
  FT: {
    ...
    drills: [
      "100 Makes Challenge",
      "Fatigue FTs (Sprint between sets)",
      "High-Stakes FT Game",
    ],
  },
  ORB: {
    ...
    drills: [
      "3-Man Tip-In Drill",
      "Physicality Box-Out Sim",
      "Second-Chance Scoring Loop",
    ],
  },
  EFG: {
    label: "Shot Selection & Finishing",
    drills: [
      "Corner 3 Progression",
      "Contact Layup Circuit",
      "Swing-Swing-Shoot Drill",
    ],
  },
  AST: {
    label: "Floor Vision",
    drills: [
      ...
      "Fast Break 3-on-2 Flow",
      "Post-Entry Read Drill",
    ],
  },
};
  ...
  const suggestions = useMemo(() => {
    ...
    if (parseFloat(teamStats.orbPct || "0") < 25) {
      focusAreas.push({
        ...
        reason: "Struggling on offensive glass",
      });
    }
    ...
    if (parseFloat(teamStats.efgPct || "0") < 45) {
      focusAreas.push({
        ...
        reason: "Poor effective shooting",
      });
    }
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary" gutterBottom>
          Based on the statistical performance of this game, we recommend
          focusing on these 3 areas in your next practice:
        </Typography>
        ...
        <Stack spacing={3} sx={{ mt: 3 }}>
          ...
          {suggestions.length === 0 && (
            <Typography
              variant="body1"
              sx={{ textAlign: "center", py: 4, fontWeight: 600 }}
            >
              Excellent game performance! Maintain current fundamentals in next
              practice.
            </Typography>
          )}
        </Stack>
