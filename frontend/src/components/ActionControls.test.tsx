...
describe("ActionControls", () => {
  ...
  it("calls onNextPeriod when Period button is clicked", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Advance to Next Period" }),
    );
  ...
  it("calls onTogglePossession when Poss button is clicked", () => {
    render(<ActionControls {...props} />);
    fireEvent.click(
      screen.getByRole("button", { name: "Change possession to Our Team" }),
    );
  ...
  it("disables buttons when isReadOnly is true", () => {
    ...
    expect(
      screen.getByRole("button", { name: "manage lineup substitutions" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "log team timeout" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "record free throws" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "undo last action" }),
    ).toBeDisabled();
    expect(
      screen.queryByRole("button", { name: "End and Save Game" }),
    ).toBeNull();
  });
  ...
  it("disables undo when recentStatsLength is 0", () => {
    ...
    expect(
      screen.getByLabelText("undo last action (no actions to undo)"),
    ).toBeDisabled();
  });
});
