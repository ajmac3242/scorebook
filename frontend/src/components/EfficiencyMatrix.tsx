            ...
            {opponents.map((opp) => (
              <TableCell key={opp.id} align="center" sx={{ fontWeight: 800 }}>
                <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                   <Avatar sx={{ width: 24, height: 24, fontSize: "0.7rem", bgcolor: "secondary.main", mb: 0.5 }}>
                     {opp.id?.split(":")[1] || "??"}
                   </Avatar>
                   <Typography variant="caption">#{opp.id?.split(":")[1] || "??"}</Typography>
                </Box>
              </TableCell>
            ))}
          </TableRow>
            ...
            <TableRow key={our.id}>
              <TableCell sx={{ fontWeight: 800 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <Typography variant="caption">{our.name.split(" ")[0]}</Typography>
                </Box>
              </TableCell>
              {opponents.map((opp) => {
                const eff = getEfficiency(our.id!, opp.id!);
                return (
                  <Tooltip key={opp.id} title={eff !== null ? `Stop %: ${eff}%` : "No data"} arrow>
                    <TableCell
                      align="center"
