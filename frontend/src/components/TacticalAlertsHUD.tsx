...
export const TacticalAlertsHUD: React.FC<TacticalAlertsHUDProps> = ({
  alerts,
  onAction: _onAction,
}) => {
        ...
        {alerts.map((alert) => (
          <Box
            ...
            sx={{
              ...
              border: `1px solid ${alert.severity === "error" ? "rgba(244, 67, 54, 0.2)" : alert.severity === "warning" ? "rgba(255, 152, 0, 0.2)" : "rgba(33, 150, 243, 0.2)"}`,
              animation:
                alert.severity === "error" ? `${pulse} 2s infinite` : "none",
            }}
          >
            <Stack direction="row" spacing={1.5} alignItems="flex-start">
              ...
              <Box sx={{ flex: 1 }}>
                <Typography
                  variant="caption"
                  sx={{ fontWeight: 700, lineHeight: 1.2, display: "block", mb: 1 }}
                >
                  {alert.message}
                </Typography>
                {(alert.type === "FOUL" || alert.type === "FATIGUE") &&
                  alert.playerId && (
                    <Button
                      size="small"
                      variant="contained"
