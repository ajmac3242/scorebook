...
Observation: A strategic audit of the live `GameMode` has exposed two critical operational hurdles within the core pillars of Scoring and Fouls. First, the `Scoreboard` lacks a numerical display for team fouls, forcing coaches to perform mental math to anticipate bonus transitions—dots are an insufficient visual proxy for high-stakes decision-making. Second, a severe logic bug in `useGameModeActions.ts` causes the `FreeThrowWorkflowDialog` to incorrectly attribute free throws to the player who *committed* the foul (the last recorded `FOUL_SHOOTING` event) rather than the player who was fouled (the shooter).

Impact: Missing numerical foul counts on the scoreboard increases cognitive load and risks tactical errors during bonus-entry windows. Incorrect free throw attribution destroys the integrity of player scoring records and invalidates efficiency data, which is a violation of our commitment to 'Mathematical Truth.'

Recommendation: Prioritize the 'Numerical Scoreboard Foul Display' and 'Corrected Free Throw Attribution' to secure the mathematical and operational floor. We must ensure the basic recording of points and fouls is 100% accurate before moving to Phase 2.
