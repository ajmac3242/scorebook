...
- **Inline Quick-Register Controls**: Added custom jersey input and "Quick-Register Jersey #[X]" action triggers to `OpponentJerseyPicker.tsx` (for opponent team) and `StatEntryDialog.tsx` (for home team).
- **IndexedDB Roster Persistence**:
...
## Opponent Team Foul Reset, Direct Score Override Sync & Period Verification Audit (September 2026)
- **Opponent Team Foul Reset Alignment**: Updated `useGameAggregator.ts` to calculate `oppFouls` using `isEventInPeriod(s.period, period, pType)`, aligning opponent team foul resets and overtime carryovers with home team rules for QUARTERS vs HALVES formats.
- **Atomic Direct Score Correction Snapshot**: Updated `handleDirectScoreOverride` in `useGameModeActions.ts` to wrap `db.stats.add` (inserting `SYSTEM_ADJUSTMENT`) and `db.games.update` (persisting updated total `teamScore` and `oppScore` snapshot fields calculated via `calculateGameResult`) inside a single Dexie read-write transaction (`db.transaction("rw", [db.stats, db.games], ...)`), preventing score snapshot desynchronizations or lag on page reloads.
- **Period-End Official Final Score Verification Audit Interlock**: Updated `VerifiedPeriodModal.tsx` to display an official score breakdown table with per-period and cumulative totals for both teams. Added a mandatory score reconciliation audit interlock (`hasScoreMismatch` warning banner and confirmation checkbox) that blocks period verification until score discrepancies are audited and confirmed.

## Live Game Clock Expiration Auto-Pause Guard (September 2026)
- **Automated Clock Expiration Freeze & Persistence**: Updated `useGameClock.ts` to ensure that when `clockSeconds` reaches 0 (via timer interval countdown or state update), `isClockRunning` is set to `false`, the buzzer is sounded (`playBuzzerSound`), and `clockTime: 0` alongside `synced: 0` and `currentPeriod` is persisted directly to `db.games` in IndexedDB.
- **Dependency & Auto-Pause Safety**: Added `gameId`, `period`, and `db` to `useGameClock.ts`'s timer effect dependency array and added error handling for database writes, ensuring clean offline persistence without unhandled rejections or clock drift on page reloads.
